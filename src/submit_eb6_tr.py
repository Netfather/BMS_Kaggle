...
Record_log_file = "train_eb6_tr-2021-05-27-00-32.log"   # 本次提交模型的是在哪个log文件被记载的
...
model_name = "eb6_trans"

if model_name == "eb6_trans":
    fold = 5
    out_dir = '../model/eb6_trans/fold{}'.format(fold)
    initial_checkpoint = out_dir + '/checkpoint/1590000_model.pth'  # None #

# 初始化日志头
logdir = "../submit/eb6_trans/fold{}".format(fold)
logger = get_logger(logdir,OutputOnConsole= True,log_initial= "sub_eb6_tr",logfilename="sub_eb6_tr_noro_1590000")


# 模型输出位
submit_dir = '../submit/eb6_trans/fold{}'.format(fold)
...
if 1:
    ...
    valid_loader  = DataLoader(
        ...
        # sampler = util.FixNumSampler(valid_dataset, 500000),
        batch_size  = 64, #32,
        drop_last   = False,
        num_workers = 8,
        pin_memory  = True,
