...
pretrained_model_path = {
    "se_resnext101_32x4d":"./utils/pretrain_model/se_resnext101_32x4d-3b2fe3d8.pth",
...
class Encoder(nn.Module):
    ...
    def __init__(self,):
        ...
        # 生成网络 并从硬盘加载权重
        model_name = 'effecient_b5'
        # print(pretrained_model_path[model_name])
        model = EfficientNet.from_pretrained('efficientnet-b5',weights_path = pretrained_model_path[model_name])
...
class Net(nn.Module):
    ...
    def forward_argmax_decode(self, image):
        ...
        image_embed = image_embed.permute(2,3,0,1).contiguous()
        image_embed = image_embed.reshape(num_pixel, batch_size, image_dim)
        image_embed = self.image_encode(image_embed)  # 此时的shape 以 batchsize 30  锚定7*7 为例子    torch.Size([49, 30, 1024])

        ## Only for test
        # print("after encode image_embed's shape: {}".format(image_embed.shape))
        ...
        if 1:
            ...
            for t in range(max_length-1):
                ...
                # 此时的l大小为 [30, 193]  0维为batchsize  1维为词典维度

                # ## Only for test
                # print("l's shape: {}".format(l.shape))
