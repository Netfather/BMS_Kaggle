...
# -------------------------------------------------------------------------------
# Name:         pdata
# Description:  此文件为 BMS分子翻译大赛的第一步  处理数据集
#               将文件给与的训练图片读入  然后找到对应的化学式
#               然后根据NLP的思路 将对应的化学式处理成 词典形式返回
# 2021年5月17日
...
# 必要的超参数
Kfold = 6  # 指定预处理图片时使用的fold数量

# 编码 化学式
util.make_train(fold=Kfold)  # 读入train文件夹下的 train.csv文件然后将化学式编码 转变为一系列的数字  这里将所有数据划分为6个fold
...
for fold_id in range(Kfold):
    ...
    # 保存为 pkl 文件
    df_train.to_pickle('../data/df_train{}.pkl'.format(fold_id))
    df_valid.to_pickle('../data/df_valid{}.pkl'.format(fold_id))
